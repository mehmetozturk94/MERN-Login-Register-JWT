...
const Login = ({login, isAuth, isloading, user}) => {
    ...
    if(isAuth && user){
        ...
        if(role === 0) return <Redirect to='/dashboard/user'></Redirect>
        if(role === 1) return <Redirect to='/dashboard/admin'></Redirect>
    }
    ...
    return (
        <Container>
            <form className='bg-white rounded-lg overflow-hidden shadow-2xl p-5  my-16 md:w-1/2 lg:w-1/3 mx-auto flex flex-col' onSubmit={onSubmit}>
                ...
                <FormInput
                title='Email'
                placeholder='mehmetozturk@example.com'
                value={email}
                ...
                <Button
                    isButton={false}
                    title='Do you need a new account?'
                    href='/register'
